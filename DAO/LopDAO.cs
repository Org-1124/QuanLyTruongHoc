   ...
   public class LopDAO
    {
       ...
       {
           string sTruyVan = "select a.IDLop, a.TenLop, a.SoLuong, a.IDLopTruong, a.IDChuNhiem, b.HoTen 'TenLopTruong', c.HoTen 'TenChuNhiem' from tblLop a, tblHocSinh b, tblGiaoVien c where b.IDHocSinh=a.IDLopTruong and c.IDGiaoVien=a.IDChuNhiem";
       ...
       public static bool XoaLop( LopDTO lop)
       {
           try
           {
               string sTruyVan = string.Format("delete from tblLop where IDLop = {0} delete tblHocSinh where IDLop = {1} ", lop.IDLop, lop.IDLop);
               con = DataProvider.KetNoi();
               DataProvider.ThucThiTruyVan(sTruyVan, con);
               DataProvider.DongKetNoi(con);
               return true;
